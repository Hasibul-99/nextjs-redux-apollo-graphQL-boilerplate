...
function ThumbTwo( props ) {
    const { productImages, index = 0 } = props;
    ...
    return (
        <div className="product-thumbs-wrap product-thumbs-two">
            <OwlCarousel adClass="product-thumbs product-thumb-carousel" options={ mainSlider15 } onChangeRef={ changeRefHandler }>
                {
                    productImages?.length ? productImages.map( ( thumb, index ) => (
                        <div className={ `product-thumb ${ index === 0 ? 'active' : '' }` } 
                            onClick={ ( e ) => { thumbActiveHandler( e, index ) } } key={ '-2-' + index }>
                            <img src={ process.env.NEXT_PUBLIC_ASSET_URI + "/" + thumb?.image_path } alt="product thumbnail" width="137" height="137" />
                        </div>
                    )) : null
                }
            </OwlCarousel>
